package org.xbib.gradle.plugin.monkeypatch
...
class MonkeyPatchPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        project.with {
            ...
            afterEvaluate {
                ...
                if (!initialized) {
                    throw new RuntimeException("monkeyPatch.target not initialized")
                }
            }
        }
    }
                ...
                jar {
                    with copySpec {
                        ...
                        eachFile { FileCopyDetails action ->
                            ...
                            boolean patched = output.classesDirs.files.any {
                                new File(it, path).exists()
                            }
                            if (!patched) {
