...
import static org.apache.lucene.testframework.util.LuceneTestCase.TEST_NIGHTLY;
import static org.apache.lucene.testframework.util.LuceneTestCase.rarely;
...
import static org.elasticsearch.testframework.ESTestCase.getTestTransportType;
...
public final class InternalTestCluster extends TestCluster {
    ...
    private static final Logger logger = Loggers.getLogger(InternalTestCluster.class);
    ...
    public InternalTestCluster(long clusterSeed, Path baseDir,
                               ...
                               boolean autoManageMinMasterNodes, int minNumDataNodes, int maxNumDataNodes,
                               String clusterName, NodeConfigurationSource nodeConfigurationSource, int numClientNodes,
                               boolean enableHttpPipelining, String nodePrefix,
                               Collection<Class<? extends Plugin>> mockPlugins, Function<Client, Client> clientWrapper) {
        super(clusterSeed);
        ...
        builder.put(RecoverySettings.INDICES_RECOVERY_RETRY_DELAY_STATE_SYNC_SETTING.getKey(),
                TimeValue.timeValueMillis(RandomNumbers.randomIntBetween(random, 20, 50)));
        defaultSettings = builder.build();
        executor = EsExecutors.newScaling("internal_test_cluster_executor", 0, Integer.MAX_VALUE, 0,
                TimeUnit.SECONDS, EsExecutors.daemonThreadFactory("test_" + clusterName), new ThreadContext(Settings.EMPTY));
    }
    ...
    private Settings getSettings(int nodeOrdinal, long nodeSeed, Settings others) {
        ...
        if (settings != null) {
            if (settings.get(ClusterName.CLUSTER_NAME_SETTING.getKey()) != null) {
                throw new IllegalStateException("Tests must not set a '" +
                        ClusterName.CLUSTER_NAME_SETTING.getKey() + "' as a node setting set '" +
    ...
    private Settings getRandomNodeSettings(long seed) {
        ...
        if (random.nextInt(10) == 0) { // 10% of the nodes have a very frequent check interval
            builder.put(SearchService.KEEPALIVE_INTERVAL_SETTING.getKey(),
                    TimeValue.timeValueMillis(10 + random.nextInt(2000)).getStringRep());
        } else if (random.nextInt(10) != 0) { // 90% of the time - 10% of the time we don't set anything
            builder.put(SearchService.KEEPALIVE_INTERVAL_SETTING.getKey(),
                    TimeValue.timeValueSeconds(10 + random.nextInt(5 * 60)).getStringRep());
        }
        if (random.nextBoolean()) { // sometimes set a
            builder.put(SearchService.DEFAULT_KEEPALIVE_SETTING.getKey(),
                    TimeValue.timeValueSeconds(100 + random.nextInt(5 * 60)).getStringRep());
        }
        ...
        if (random.nextBoolean()) {
            builder.put(MappingUpdatedAction.INDICES_MAPPING_DYNAMIC_TIMEOUT_SETTING.getKey(),
                    new TimeValue(RandomNumbers.randomIntBetween(random, 10, 30), TimeUnit.SECONDS));
        }
        ...
        if (random.nextBoolean()) {
            builder.put(TcpTransport.PING_SCHEDULE.getKey(),
                    RandomNumbers.randomIntBetween(random, 100, 2000) + "ms");
        }

        if (random.nextBoolean()) {
            builder.put(ScriptService.SCRIPT_CACHE_SIZE_SETTING.getKey(),
                    RandomNumbers.randomIntBetween(random, 0, 2000));
        }
        if (random.nextBoolean()) {
            builder.put(ScriptService.SCRIPT_CACHE_EXPIRE_SETTING.getKey(),
                    TimeValue.timeValueMillis(RandomNumbers.randomIntBetween(random, 750, 10000000)).getStringRep());
        }
    ...
    public synchronized void ensureAtMostNumDataNodes(int n) throws IOException {
        ...
        final Stream<NodeAndClient> collection =
            n == 0 ? nodes.values().stream() : nodes.values().stream().filter(new DataNodePredicate()
                    .and(new MasterNodePredicate(getMasterName()).negate()));
    ...
    private NodeAndClient buildNode(int nodeId, long seed, Settings settings,
        ...
        final boolean usingSingleNodeDiscovery =
                DiscoveryModule.DISCOVERY_TYPE_SETTING.get(finalSettings.build()).equals("single-node");
    ...
    public synchronized Client client(final Predicate<Settings> filterPredicate) {
        ensureOpen();
        final NodeAndClient randomNodeAndClient = getRandomNodeAndClient(nodeAndClient ->
                filterPredicate.test(nodeAndClient.node.settings()));
        if (randomNodeAndClient != null) {
            return randomNodeAndClient.client(random);
        }
        return null;
    ...
    private final class NodeAndClient implements Closeable {
        ...
        Client client(Random random) {
            ...
            if (nextDouble < transportClientRatio) {
                if (logger.isTraceEnabled()) {
                    logger.trace("Using transport client for node [{}] sniff: [{}]",
                            node.settings().get("node.name"), false);
                }
        ...
        private Client getOrBuildTransportClient() {
            if (transportClient == null) {
                /* no sniff client for now - doesn't work will all tests since it might
                 * throw NoNodeAvailableException if nodes are shut down.
                 * we first need support of transportClientRatio as annotations or so
                 */
                transportClient = new TransportClientFactory(false,
                        nodeConfigurationSource.transportClientSettings(), baseDir,
                        nodeConfigurationSource.transportClientPlugins()).client(node, clusterName);
            }
        ...
        void recreateNodeOnRestart(RestartCallback callback, boolean clearDataIfNeeded, int minMasterNodes) throws Exception {
            ...
            if (minMasterNodes >= 0) {
                assert DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING.exists(newSettings.build()) == false :
                        "min master nodes is auto managed";
        ...
        private void createNewNode(final Settings newSettings) {
            ...
            Settings finalSettings = Settings.builder().put(node.originalSettings()).put(newSettings)
                    .put(NodeEnvironment.NODE_ID_SEED_SETTING.getKey(), newIdSeed).build();
                    ...
                    for (IndexShard indexShard : indexService) {
                        ...
                        if (operations.size() > 0) {
                            throw new AssertionError(
                                    "shard " + indexShard.shardId() + " on node [" +
                                            nodeAndClient.name + "] has pending operations:\n --> " +
    ...
    private synchronized void startAndPublishNodesAndClients(List<NodeAndClient> nodeAndClients) {
        if (nodeAndClients.size() > 0) {
            ...
            List<Future<?>> futures = nodeAndClients.stream().map(node ->
                    executor.submit(node::startNode)).collect(Collectors.toList());
            try {
     ...
     * Restarts a random node in the cluster and calls the callback during restart.
     */
    private synchronized void restartRandomNode(Predicate<NodeAndClient> predicate, RestartCallback callback)
            throws Exception {
        ensureOpen();
    ...
    public synchronized void fullRestart(RestartCallback callback) throws Exception {
        ...
        for (NodeAndClient nodeAndClient : startUpOrder) {
            ...
            nodeAndClient.recreateNodeOnRestart(callback, false, autoManageMinMasterNodes ?
                    getMinMasterNodes(getMasterNodesCount()) : -1);
        }
    ...
    public synchronized String startNode(Settings settings) {
        final int defaultMinMasterNodes = getMinMasterNodes(getMasterNodesCount() +
                (Node.NODE_MASTER_SETTING.get(settings) ? 1 : 0));
    ...
    public synchronized List<String> startMasterOnlyNodes(int numNodes, Settings settings) {
        Settings settings1 = Settings.builder().put(settings).put(Node.NODE_MASTER_SETTING.getKey(), true)
    ...
    public synchronized List<String> startDataOnlyNodes(int numNodes, Settings settings) {
        Settings settings1 = Settings.builder().put(settings).put(Node.NODE_MASTER_SETTING.getKey(), false)
        ...
        if (getMasterNodesCount() > 0) {
            ...
            try {
                assertAcked(client().admin().cluster().prepareUpdateSettings().setTransientSettings(
                    Settings.builder().put(DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING.getKey(), minMasterNodes).build()
                ));
            } catch (Exception e) {
    ...
    public void clearDisruptionScheme(boolean ensureHealthyCluster) {
        if (activeDisruptionScheme != null) {
            ...
            logger.info("Clearing active scheme {}, expected healing time {}", activeDisruptionScheme,
                    expectedHealingTime);
    ...
    synchronized String routingKeyForShard(Index index, int shard, Random random) {
        ...
        for (NodeAndClient n : nodes.values()) {
            ...
            if (indexService != null) {
                assertThat(indexService.getIndexSettings().getSettings()
                        .getAsInt(IndexMetaData.SETTING_NUMBER_OF_SHARDS, -1), greaterThan(shard));
                OperationRouting operationRouting = clusterService.operationRouting();
                while (true) {
                    String routing = RandomStrings.randomAsciiOfLength(random, 10);
                    final int targetShard = operationRouting.indexShards(clusterService.state(),
                            index.getName(), null, routing).shardId().getId();
    ...
    public static class RestartCallback {
        ...
        /**
         * If this returns {@code true} all data for the node with the given node name will be cleared including
         * gateways and all index data. Returns {@code false} by default.
         */
            ...
            for (NodeAndClient nodeAndClient : nodes.values()) {
                ...
                try {
                    ...
                    assertBusy(() -> {
                        ...
                        assertThat("All incoming requests on node [" + nodeAndClient.name +
                                "] should have finished. Expected 0 but got " +
