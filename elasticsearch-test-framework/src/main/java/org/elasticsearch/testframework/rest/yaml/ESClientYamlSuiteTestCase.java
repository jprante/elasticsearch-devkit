package org.elasticsearch.testframework.rest.yaml;
...
import org.elasticsearch.testframework.common.io.ResourceUtilsForTesting;
import org.elasticsearch.testframework.rest.ESRestTestCase;
import org.elasticsearch.testframework.rest.yaml.restspec.ClientYamlSuiteRestApi;
import org.elasticsearch.testframework.rest.yaml.restspec.ClientYamlSuiteRestSpec;
import org.elasticsearch.testframework.rest.yaml.section.ClientYamlTestSection;
import org.elasticsearch.testframework.rest.yaml.section.ClientYamlTestSuite;
import org.elasticsearch.testframework.rest.yaml.section.DoSection;
import org.elasticsearch.testframework.rest.yaml.section.ExecutableSection;
...
public abstract class ESClientYamlSuiteTestCase extends ESRestTestCase {
    ...
    private static final String TESTS_PATH = "restapispec/test";
    private static final String SPEC_PATH = "restapispec/api";
    ...
    public void initAndResetContext() throws Exception {
        if (restTestExecutionContext == null) {
            ...
            ClientYamlSuiteRestSpec restSpec =
                    ClientYamlSuiteRestSpec.load(ClientYamlSuiteRestSpec.class.getClassLoader(), SPEC_PATH);
    ...
    public static Iterable<Object[]> createParameters(ClassLoader classLoader) throws Exception {
        String[] paths = resolvePathsProperty(REST_TESTS_SUITE, ""); // default to all tests under the test root
        List<Object[]> tests = new ArrayList<>();
        Map<String, Collection<URL>> yamlSuites = loadSuites(classLoader, paths);
        // yaml suites are grouped by directory (effectively by api)
        for (String api : yamlSuites.keySet()) {
            List<URL> yamlFiles = new ArrayList<>(yamlSuites.get(api));
            for (URL yamlFile : yamlFiles) {
    ...
    /** Find all yml suites that match the given list of paths from the root test path. */
    // pkg private for tests
    public static Map<String, Collection<URL>> loadSuites(ClassLoader classLoader, String... paths) throws Exception {
        Map<String, Collection<URL>> suites = ResourceUtilsForTesting.getResourceGroups(classLoader, TESTS_PATH, Arrays.asList(paths));
        Logger logger = Loggers.getLogger(ESClientYamlSuiteTestCase.class);
