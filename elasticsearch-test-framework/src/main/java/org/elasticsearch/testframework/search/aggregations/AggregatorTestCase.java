package org.elasticsearch.testframework.search.aggregations;
...
import org.apache.lucene.testframework.search.AssertingIndexSearcher;
...
import static org.elasticsearch.testframework.InternalAggregationTestCase.DEFAULT_MAX_BUCKETS;
...
public abstract class AggregatorTestCase extends ESTestCase {
     ...
     * @param aggregationBuilder the aggregation builder
     * @param indexSearcher the index searcher
     * @param indexSettings  the index settings
     * @param bucketConsumer the bucket consumer
     * @param fieldTypes the field types
     * @return aggregator factory
     * @throws IOException if factory can not be created
     */
    ...
    protected SearchContext createSearchContext(IndexSearcher indexSearcher, IndexSettings indexSettings) {
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(searchContext).addReleasable(any(), any());
    ...
    protected QueryShardContext queryShardContextMock(MapperService mapperService, MappedFieldType[] fieldTypes,
        ...
        Mockito.doCallRealMethod().when(queryShardContext).setIsFilter(anyBoolean());
     ...
     *
     * @param <A> type parameter for aggregations
     * @param <C> type parameter for aggregator
     * @param searcher searcher
     * @param query query
     * @param builder builder
     * @param maxBucket max bucket
     * @param fieldTypes field types
     * @return aggregation
     ...
     * like {@link LuceneTestCase#newSearcher(IndexReader)}, which can't be used because it also
     ...
     * like {@link LuceneTestCase#wrapReader(IndexReader)}, which can't be used because it also
