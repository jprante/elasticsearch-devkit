package org.elasticsearch.testframework.engine;
...
/**
 * Support class to build MockEngines like {@link org.elasticsearch.testframework.engine.MockInternalEngine}
...
public final class MockEngineSupport {
     ...
     * This is disabled by default ie. {@code 0.0d} since reader wrapping is insanely
    ...
    public CloseAction flushOrClose(CloseAction originalAction) throws IOException {
        if (closing.compareAndSet(false, true)) {
            // only do the random thing if we are the first call to this since super.flushOnClose()
            // calls #close() again and then we might end up with a stackoverflow.
    ...
    public Engine.Searcher wrapSearcher(String source, Engine.Searcher engineSearcher) {
        ...
        // on release otherwise the reader will be closed too early. - good news, stuff will
        // fail all over the place if we don't get this right here
