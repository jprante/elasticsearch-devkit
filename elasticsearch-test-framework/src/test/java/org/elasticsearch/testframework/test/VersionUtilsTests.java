package org.elasticsearch.testframework.test;
...
public class VersionUtilsTests extends ESTestCase {
    ...
    public void testGradleVersionsMatchVersionUtils() {
        ...
        List<String> releasedIndexCompatible = VersionUtils.allReleasedVersions().stream()
                .filter(Version::isRelease)
                .filter(v -> !Version.CURRENT.equals(v))
                .map(Object::toString)
                .collect(toList());
        List<String> indexCompatibleReleased = indexCompatible.released.stream()
                .filter(v -> !Version.CURRENT.toString().equals(v))
                .collect(toList());
        assertEquals(releasedIndexCompatible, indexCompatibleReleased);
        ...
        List<String> releasedWireCompatible = VersionUtils.allReleasedVersions().stream()
                .filter(Version::isRelease)
                .filter(v -> !Version.CURRENT.equals(v))
                .filter(v -> v.onOrAfter(minimumCompatibleVersion))
                .map(Object::toString)
                .collect(toList());
        List<String> wireCompatibleReleased = wireCompatible.released.stream()
                .filter(v -> !Version.CURRENT.toString().equals(v))
                .collect(toList());
        assertEquals(releasedWireCompatible, wireCompatibleReleased);
