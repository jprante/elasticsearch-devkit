...
public class RandomElements<T> {
    ...
    /**
     * In case you have a collection and then couple of other elements you want to sample from too,
     * but you don't want to create a collection that includes all of them combined.
     *
     ...
     */
    public T sample() {
        int index = RandomApi.generateInteger(size() - 1);
    ...
    public List<T> sample(int nToReturn) {
        ...
        List<T> result = new ArrayList<>(nToReturn);
        for (int i = 0; i < nToReturn; i++) {
            int index = RandomApi.generateInteger(elements.size() - 1);
