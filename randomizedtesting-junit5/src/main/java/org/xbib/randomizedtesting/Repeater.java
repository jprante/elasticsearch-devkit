...
/**
 * If the resulting string should have repeats, this class can do this.
 * Note, that often there are separators between the repeats which should be trimmed
 * at the end - and this trimming is the default behaviour. To switch it off
...
public class Repeater {
    ...
    public String times(int min, int max) {
        ...
        return times(RandomApi.generateInteger(min, max));
    }

    public String times(int exactNumberOfTimes) {
        if (exactNumberOfTimes < 0) {
            throw new IllegalArgumentException("I cannot repeat string negative number of times");
        }
