...
/**
 * Checks that all tests in a directory are named according to our naming conventions.
 * This is important because tests that do not follow our conventions aren't run by gradle.
 * This was once a glorious unit test but now that Elasticsearch is a multi-module project it must be
 * a class with a main method so gradle can call it for each project.
 * This has the advantage of allowing gradle to calculate when it is
 * {@code UP-TO-DATE} so it can be skipped if the compiled classes haven't changed.
 * This is useful on large modules for which checking all the modules can be slow.
 *
 * Annoyingly, this cannot be tested using standard unit tests because to do so you'd have to declare classes
 * that violate the rules. That would cause the test fail which would prevent the build from passing.
 * So we have to make a mechanism for removing those test classes.
 * Now that we have such a mechanism it isn't much work to fail the process if we don't detect the offending classes.
...
public class NamingConventionsCheck {
    ...
    public void checkTests(Path rootPath, boolean skipTestsInDisguised) throws IOException {
        Files.walkFileTree(rootPath, new TestClassVisitor() {
            ...
            protected void visitTestClass(Class<?> clazz) {
                if (!skipTestsInDisguised && integTestClass.isAssignableFrom(clazz)) {
    ...
    private static void assertViolation(String className, Set<Class<?>> set) {
        ...
        if (!set.remove(loadClassWithoutInitializing(className))) {
